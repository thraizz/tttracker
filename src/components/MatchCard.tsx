      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className={`flex items-center justify-between p-3 rounded-lg transition-colors ${
            isCompleted && match.winner?.id === match.player1.id 
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${
                isCompleted && match.winner?.id === match.player1.id ? 'text-victory-gold' : ''
              }`}>
                {match.player1.name}
              </span>
            </div>
            ...
            {isCompleted && match.score && (
              <span className={`font-bold text-lg ${
                match.winner?.id === match.player1.id ? 'text-victory-gold' : 'text-muted-foreground'
              }`}>
          ...
          {/* Player 2 */}
          <div className={`flex items-center justify-between p-3 rounded-lg transition-colors ${
            isCompleted && match.winner?.id === match.player2.id 
              ? 'bg-victory-gold/10 border border-victory-gold/30' 
              : 'bg-muted/50'
          }`}>
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${
                isCompleted && match.winner?.id === match.player2.id ? 'text-victory-gold' : ''
              }`}>
                {match.player2.name}
              </span>
            </div>
            ...
            {isCompleted && match.score && (
              <span className={`font-bold text-lg ${
                match.winner?.id === match.player2.id ? 'text-victory-gold' : 'text-muted-foreground'
              }`}>
